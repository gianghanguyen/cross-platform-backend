...
adminCategoryRouter.post('/', tokenExtractor('ADMIN'), async (req: Request, res: Response) => {
...
adminCategoryRouter.delete('/:id', tokenExtractor('ADMIN'), async (req: Request, res: Response) => {
  const { id } = req.params;
  res.json(await deleteCategory({ id: parseInt(id) }));
});

adminCategoryRouter.put('/:id', tokenExtractor('ADMIN'), async (req: Request, res: Response) => {
  const { id } = req.params;
